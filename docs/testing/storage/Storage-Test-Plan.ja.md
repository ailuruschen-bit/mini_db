...
- [ ] `blankPage()` — ゼロ初期化された `*SlottedPage`。
- [ ] `knownPage()` — 既知のヘッダー / スロット / タプルのバイトを事前設定したページ。
...
- [ ] 往復: set 後に get が同じ値を返す（全フィールド）。
- [ ] ゴールデンオフセット / バイト順: set 後、当該範囲の生バイトがビッグエンディアン表現を保持する。
- [ ] フィールド独立性: あるフィールドの書き込みが隣接フィールドを変えない。
- [ ] 境界値: `0` と型の最大値（u16 / u32 / u64）。

## 2. SlotEntry（`slot_entry_test.go`、ホワイトボックス）— 最高リスク（4 バイトのビットパック）

レイアウト: `Offset`（ビット 31–17）、`Length`（ビット 16–2）、`Flags`（ビット 1–0）。

- [ ] `Offset` / `Length` / `Flag` の往復。
- [ ] **ビットフィールド独立性:** あるフィールドを全 1 にし、別のフィールドを set して、最初のフィールドが不変であることを検証（全組み合わせ）。
- [ ] 境界値: `0`、最大値（`Offset`/`Length` = 32767、`Flag` = 3）。
- [ ] エラーパス: `SetOffset`/`SetLength`（> 32767）と `SetFlag`（> 3）はエラーを返し、書き込まない。
- [ ] ゴールデン: 既知の offset/length/flag → 期待する生 4 バイト。

## 3. TupleHeader（`tuple_test.go`、ホワイトボックス）— 12 バイトレイアウト

レイアウト: `t_xmin[0:4]` `t_xmax[4:8]` `flags`(6b)+`col_count`(10b) `[8:10]` `t_hoff[10]` 予約 `[11]`。

- [ ] `TupleHeader()` が先頭 12 バイトのビューを返す。
- [ ] `TxMin` / `TxMax` の往復とゴールデンオフセット。
- [ ] `Flags` / `ColumnCount` のパック独立性（一方の set が他方を保持）、境界値（flags = 63、col_count = 1023）、エラーパス（> 最大値）。
- [ ] `Hoff` の往復。予約バイト `[11]` がすべての setter で不変。
- [ ] `HasNull` が `FlagHasNull` ビットを反映する。
...
- [ ] 公開 API でページを組み立て（ヘッダー + スロット + タプル）→ `data` を取り出す → `NewSlottedPage` で再構築 → すべての getter が同じ値を読み戻す。
